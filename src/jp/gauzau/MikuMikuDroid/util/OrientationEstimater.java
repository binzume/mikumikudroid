...
public class OrientationEstimater {
	...
	private final static float G = 9.765f; // SensorManager.GRAVITY_EARTH
	...
	public void onSensorEvent(SensorEvent event) {
		...
		if (gyroVec.length() < 0.2f && Math.abs(accVecN.length() - G) < 0.5f) {
			...
			if (theta > 0) {
				...
				Matrix.setRotateM(rotationMatrix_t1, 0, theta * 180 / PI * 0.05f, cross[0], cross[1], cross[2]);
				Matrix.multiplyMM(rotationMatrix_t2, 0, rotationMatrix_t1, 0, rotationMatrix, 0);
				float tm[] = rotationMatrix_t2;
				rotationMatrix_t2 = rotationMatrix;
				rotationMatrix = tm;
		...
		System.arraycopy(rotationMatrix, 0, outputRotationMatrix, 0, 16);
		Matrix.rotateM(outputRotationMatrix, 0, xRotationBase, 1, 0, 0);
		Matrix.rotateM(outputRotationMatrix, 0, yRotationBase, 0, 1, 0);
	}
